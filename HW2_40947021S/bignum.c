...
int32_t compare(const sBigNum num01, const sBigNum num02)
{
	if (num01.sign == -1 && num02.sign == 1)
		...
		return num01.sign;
	else if (num02.size > num01.size)
		return -num01.sign;
	else
	{
		for (int i = num01.size - 1; i >= 0; --i)
		{
			if (num01.num[i] > num02.num[i])
				return num01.sign;
			else if (num02.num[i] > num01.num[i])
				return -num01.sign;
		}
	}
	return 0;
...
void subtract(sBigNum *pResult, const sBigNum num01, const sBigNum num02)
{
	...
	for (size_t i = 0; i < max_size; ++i)
	{
		...
		pResult->num[i] -= carry;
		if (pResult->num[i] < 0)
			carry = 1, pResult->num[i] += 10;
		else
			carry = 0;
	}
	if (carry == 1 && pResult->num[max_size] < 0)
		pResult->num[max_size++] = -1;
	while (max_size != 1 && pResult->num[max_size - 1] == 0)
		--max_size;
	if (pResult->num[0] == 0 && max_size == 1)
		pResult->size = 1, pResult->sign = 1;
	else
		pResult->size = max_size;
	if (pResult->num[max_size - 1] < 0)
		pResult->num[max_size - 1] = -pResult->num[max_size - 1], pResult->sign = -1;
}
...
void multiply(sBigNum *pResult, const sBigNum num01, const sBigNum num02)
{
	...
	if (pResult->num[pResult->size] != 0)
		++pResult->size;
	while (pResult->size != 1 && pResult->num[pResult->size - 1] == 0)
		--pResult->size;
	if (pResult->num[0] == 0 && pResult->size == 1)
		pResult->size = 1, pResult->sign = 1;
}

void mult(sBigNum *pResult, const sBigNum num01, int n)
{
	tozero(pResult);
	...
	while (carry != 0)
		pResult->num[pResult->size++] = carry % 10, carry /= 10;
}
...
void divide(sBigNum *pQuotient, sBigNum *pRemainder, const sBigNum num01, const sBigNum num02)
{
	...
	for (; index >= 0; --index)
	{
		for (int i = 1; i <= 9; ++i)
		{
			...
			if (pQuotient->size - 1 == index && pQuotient->num[index] == 0)
				--pQuotient;
		}
		if (eqflag == 1)
		{
	...
	if (pQuotient->num[pQuotient->size - 1] != 0)
		pQuotient->sign = tsign;
	else if (pQuotient->size != 1)
		--pQuotient->size;
}
